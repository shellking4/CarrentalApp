...
@section('content')
    <div class="flex justify-center mt-5">
        <div class="w-8/12 bg-white p-6 rounded-lg">
            <h5 class="flex justify-center">UTILISATEURS</h5>
        </div>
    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-6">
                                            <form action="{{ route('user_update_view', $user) }}" method="get" class="">
                                                @csrf
                                                <button type="submit"><span class="text-success mt-md-2 mt-4 fas fa-edit"></span></button>
                                            </form>
                                        ...
                                        <div class="col-6">
                                            <form action="{{ route('user_delete', $user) }}" method="post" class="">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"><span class="fas fa-trash mt-4 mt-md-2 text-danger"></span></button>
                                            </form>
                                        </div>
                                    </div>
                                </td>
